...
export class OrderService {
  ...
  async create(actor: Actor, input: OrderInput) { requireWriter(actor); const order = await this.repo.create(actor, input); if (!order) throw new AppError(404, "NOT_FOUND", "Cliente o producto no disponible."); this.realtime?.publish({ organizationId: actor.organizationId!, type: "order.updated", resourceId: order.id }); return order; }
